...
import {
    IUser, IUserQuery, IUserSharable, IUserStatuses, ROLE, User,
} from '../models/user';
...
export const isValidRole = (submittedRole: string):[string, number]|void => {
    const newRole = Object.entries(ROLE).filter(p => (p[0] === submittedRole && isNaN(parseInt(p[0]))));
                ...
                if (req.requestor) {
                    try {
                        if (req.requestor.role === ROLE.GOD) {
                            ...
                            return 'other';
                        } if (req.requestor._id.toString() === user._id.toString() || (req.requestor.role < user.role && req.requestor.role === ROLE.ADMIN)) {
                            user.lastModified = dayjs.utc().toDate();
                            user.statuses.markedForDeletion = true;
